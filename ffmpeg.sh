...
sudo apt-get update
sudo apt-get -y --force-yes install  lame libmp3lame0 libmp3lame-dev libx264-142 libx264-dev x264 autoconf automake build-essential libass-dev libfreetype6-dev libsdl1.2-dev libtheora-dev libtool libva-dev libvdpau-dev libvorbis-dev libxcb1-dev libxcb-shm0-dev libxcb-xfixes0-dev pkg-config texi2html zlib1g-dev mercurial cmake
...
#cd x264-*/
#PATH="$HOME/bin:$PATH" ./configure --prefix="$HOME/ffmpeg_build" --bindir="$HOME/bin" --enable-static && PATH="$HOME/bin:$PATH" make && sudo make install && make distclean; cd ..
